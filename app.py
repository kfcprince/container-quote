...
# 语言选择器放在页面最顶部
lang_mode = st.radio("Language / 语言", ["Bilingual / 双语", "English Only"], horizontal=True, label_visibility="collapsed")

# 魔法翻译函数：如果是“纯英文”模式，自动砍掉斜杠后面的中文
def f_lang(text):
    if not isinstance(text, str):
        return text
    if lang_mode == "English Only" and "/" in text:
...
def t_cat(k): return f_lang(TRANS.get(k, k))
def t_item(k): return f_lang(TRANS.get(k, k))
...
if admin_pwd == "HUAhan807810":
    ...
    markup_rate = st.sidebar.number_input("Markup Rate", 1.0, 2.5, 1.2, 0.05)
...
# 主界面
# ==========================================
st.title(f_lang("📋 Container House Quotation / 集装箱房屋报价单"))
bill = []

# --- 1. Basic ---
st.subheader(f_lang("1. Basic / 基础配置"))
c1, c2 = st.columns(2)
with c1:
    size_opts = ['20FT', 'X-Folding / X折叠', 'F700', '10FT', '15FT', '30FT', '40FT']
    size_sel = st.selectbox(f_lang("Size / 房型尺寸"), size_opts, format_func=f_lang)
    if "X-Folding" in size_sel:
        size = "X折叠"
    else:
        size = size_sel
        
    bill.append({"Cat": t_cat("基础"), "Item": t_item("基础箱体"), "Spec": f_lang(size_sel), "Qty": 1, "RMB": get_p('基础箱体', size)})
...
with c2:
    ...
    layout = st.selectbox(f_lang("Layout / 内部布局"), opts, format_func=f_lang)
    layout_cn = get_cn(layout)
    if layout_cn not in ['空箱', '房间数量定制']:
        bill.append({"Cat": t_cat("基础"), "Item": t_item("内部布局"), "Spec": f_lang(layout), "Qty": 1, "RMB": get_p(layout_cn, size)})

# --- 2. Decoration ---
st.markdown("---")
st.subheader(f_lang("2. Decoration / 装修"))
is_x = (size == "X折叠")
is_f700 = (size == "F700")
...
with c1:
    ...
    ins = st.selectbox(f_lang("Insulation / 保温材料"), ins_opts, format_func=f_lang)
    ins_cn = get_cn(ins)
    if ins_cn != '岩棉 50mm': bill.append({"Cat": t_cat("装修"), "Item": t_item("保温升级"), "Spec": f_lang(ins), "Qty": 1, "RMB": get_p(ins_cn, size)})
    
    st.selectbox(f_lang("Frame Color / 框架颜色"), ['White / 白色', 'Black / 黑色', 'Grey / 灰色', 'Custom / 定制'], format_func=f_lang)
    
    wall_col_spec = st.selectbox(f_lang("Special Wall Color / 墙板特殊颜色"), ["No / 不需要", "Yes / 需要"], disabled=is_x, format_func=f_lang)
    if "Yes" in wall_col_spec:
         bill.append({"Cat": t_cat("装修"), "Item": t_item("墙板特殊颜色"), "Spec": f_lang(wall_col_spec), "Qty": 1, "RMB": get_p('墙板特殊颜色', size)})

with c2:
    in_wall = st.selectbox(f_lang("Inner Wall / 内墙"), ['Normal / 普通内墙板', 'Carbon Crystal(8mm) / 碳晶板(8mm)', 'Bamboo Fiber / 竹木纤维板', 'Graphene Wall Board / 石墨烯墙板'], format_func=f_lang)
    in_wall_cn = get_cn(in_wall)
    if in_wall_cn != '普通内墙板': bill.append({"Cat": t_cat("装修"), "Item": t_item("内墙升级"), "Spec": f_lang(in_wall), "Qty": 1, "RMB": get_p(in_wall_cn, size)})
    
    out_wall = st.selectbox(f_lang("Outer Wall / 外墙"), ['Normal / 普通外墙板', 'Metal Carved / 金属雕花板', 'WPC Great Wall / 长城板', 'WPC Wall Board / WPC墙板'], disabled=(is_x or '长城板' in RESTRICTED_FOR_X and is_x), format_func=f_lang)
    out_wall_cn = get_cn(out_wall)
    if out_wall_cn != '普通外墙板' and not is_x: bill.append({"Cat": t_cat("装修"), "Item": t_item("外墙升级"), "Spec": f_lang(out_wall), "Qty": 1, "RMB": get_p(out_wall_cn, size)})

with c3:
    floor = st.selectbox(f_lang("Floor / 地板"), ['Vinyl(2mm) / 地板革(2mm)', 'SPC(4cm) / 石塑锁扣地板(4cm)'], format_func=f_lang)
    floor_cn = get_cn(floor)
    bill.append({"Cat": t_cat("装修"), "Item": t_item("地板升级"), "Spec": f_lang(floor), "Qty": 1, "RMB": get_p(floor_cn, size)})
    
    heating = st.selectbox(f_lang("Heating / 供暖"), ['No / 无', 'Graphene Floor Heating / 石墨烯地热'], disabled=is_x, format_func=f_lang)
    heating_cn = get_cn(heating)
    if heating_cn != '无' and not is_x:
        bill.append({"Cat": t_cat("装修"), "Item": f_lang("Graphene Floor Heating / 石墨烯地热"), "Spec": "Yes", "Qty": 1, "RMB": get_p('石墨烯地热', size)})
    ...
    roof_panel = st.selectbox(f_lang("Roof Panel Upgrade / 顶板升级"), roof_panel_opts, disabled=is_x, format_func=f_lang)
    roof_panel_cn = get_cn(roof_panel)
    if roof_panel_cn != '无' and not is_x:
        bill.append({"Cat": t_cat("升级"), "Item": f_lang("Roof Panel / 顶板升级"), "Spec": f_lang(roof_panel), "Qty": 1, "RMB": get_p(roof_panel_cn, size)})

# --- 3. Doors & Windows ---
st.markdown("---")
st.subheader(f_lang("3. Doors & Windows / 门窗"))
c1, c2, c3 = st.columns(3)
with c1:
    d_main_opts = [
    ...
    d_main = st.selectbox(f_lang("Main Door / 入户门"), d_main_opts, format_func=f_lang)
    d_color = st.selectbox(f_lang("Door Color / 颜色"), ['Black / 黑色', 'White / 白色', 'Grey / 灰色'], format_func=f_lang)
    full_spec = f"{f_lang(d_main)} (Color: {f_lang(d_color)})"
...
with c2:
    ...
    d_inner = st.selectbox(f_lang("Inner Door / 室内门"), d_inner_opts, format_func=f_lang)
    
    d_inner_qty = st.number_input(f_lang("Inner Door Qty / 室内门数量"), 0, 10, 1 if "Standard" not in d_inner else 0)
    d_inner_cn = get_cn(d_inner)
    if d_inner_cn != '标配室内门' and d_inner_qty > 0: 
        bill.append({"Cat": t_cat("门窗"), "Item": t_item("室内门"), "Spec": f_lang(d_inner), "Qty": d_inner_qty, "RMB": get_p(d_inner_cn, size)})
...
with c3:
    ...
    win = st.selectbox(f_lang("Window / 窗户"), win_opts, format_func=f_lang)
    w_qty = st.number_input(f_lang("Window Qty / 窗户数量"), 0, 10, 2)
    if w_qty > 0: bill.append({"Cat": t_cat("门窗"), "Item": t_item("窗户"), "Spec": f_lang(win), "Qty": w_qty, "RMB": get_p(get_cn(win), size)})
    
    scr = st.selectbox(f_lang("Screen / 纱窗"), ['No / 不要', 'Common / 普通网', 'Steel / 金刚网'], format_func=f_lang)
    scr_cn = get_cn(scr)
    if '不要' not in scr_cn: bill.append({"Cat": t_cat("门窗"), "Item": t_item("纱窗"), "Spec": f_lang(scr), "Qty": w_qty if w_qty > 0 else 1, "RMB": get_p(scr_cn, size)})

# --- 4. Kitchen & Bath ---
st.markdown("---")
st.subheader(f_lang("4. Kitchen & Bath / 厨卫"))
...
with c1:
    ...
    bath = st.selectbox(f_lang("Bathroom / 卫生间"), bath_opts, disabled=disable_kb, format_func=f_lang)
    bath_cn = get_cn(bath)
    if bath_cn != '无' and not disable_kb: bill.append({"Cat": t_cat("厨卫"), "Item": t_item("卫浴"), "Spec": f_lang(bath), "Qty": 1, "RMB": get_p(bath_cn, size)})
    
    cab_opts = ['None / 无', 'L-Cabinet+Sink / L橱柜+洗碗池', 'Black L-Cabinet+Sink / 黑色L橱柜+洗碗池', 'Straight Cabinet / 一字型橱柜']
    cab = st.selectbox(f_lang("Cabinet / 橱柜"), cab_opts, disabled=disable_kb, format_func=f_lang)
    cab_cn = get_cn(cab)
    if cab_cn != '无' and not disable_kb: bill.append({"Cat": t_cat("厨卫"), "Item": t_item("橱柜"), "Spec": f_lang(cab), "Qty": 1, "RMB": get_p(cab_cn, size)})

with c2:
    col_a, col_b = st.columns(2)
    with col_a:
        opt_heater = st.selectbox(f_lang("Water Heater / 热水器"), ["No / 不需要", "Yes / 需要"], disabled=disable_kb, format_func=f_lang)
        if "Yes" in opt_heater and not disable_kb: bill.append({"Cat": t_cat("厨卫"), "Item": t_item("热水器"), "Spec": "Yes", "Qty": 1, "RMB": get_p('热水器', size)})
        
    with col_b:
        opt_fan = st.selectbox(f_lang("Exhaust Fan / 排气扇"), ["No / 不需要", "Yes / 需要"], disabled=disable_kb, format_func=f_lang)
        if "Yes" in opt_fan and not disable_kb: bill.append({"Cat": t_cat("厨卫"), "Item": t_item("排气扇"), "Spec": "Yes", "Qty": 1, "RMB": get_p('排气扇(200*200)', size)})
        
        opt_h_cab = st.selectbox(f_lang("Hanging Cab / 吊柜"), ["No / 不需要", "Yes / 需要"], disabled=disable_kb, format_func=f_lang)
...
st.markdown("---")
st.subheader(f_lang("5. Upgrades & Structure / 结构与升级"))
...
with c1:
    if "Yes" in st.selectbox(f_lang("Roof Waterproof / 屋顶防水"), ["No / 不需要", "Yes / 需要"], disabled=disable_struct, format_func=f_lang):
        bill.append({"Cat": t_cat("升级"), "Item": t_item("屋顶防水"), "Spec": "Yes", "Qty": 1, "RMB": get_p('屋顶全贴防水卷材', size)})
    
    if "Yes" in st.selectbox(f_lang("PU Panel 75 / 聚氨酯板75"), ["No / 不需要", "Yes / 需要"], disabled=disable_struct, format_func=f_lang):
        bill.append({"Cat": t_cat("升级"), "Item": t_item("聚氨酯板75"), "Spec": "Yes", "Qty": 1, "RMB": get_p('聚氨酯板75', size)})
        
    if "Yes" in st.selectbox(f_lang("Bottom PU(4cm) / 底部保温(4cm)"), ["No / 不需要", "Yes / 需要"], disabled=disable_struct, format_func=f_lang):
        bill.append({"Cat": t_cat("升级"), "Item": t_item("底部保温(4cm)"), "Spec": "Yes", "Qty": 1, "RMB": get_p('聚氨酯底部保温(4cm)', size)})
        
    if "Yes" in st.selectbox(f_lang("Bottom PU(Block) / 底部保温块"), ["No / 不需要", "Yes / 需要"], disabled=disable_struct, format_func=f_lang):
        bill.append({"Cat": t_cat("升级"), "Item": t_item("底部保温(块)"), "Spec": "Yes", "Qty": 1, "RMB": get_p('聚氨酯底部保温(块)', size)})

with c2:
    g_wall_opt = st.selectbox(f_lang("Glass Wall / 玻璃幕墙"), ["No / 不需要", "Yes / 需要"], disabled=disable_struct, format_func=f_lang)
    if "Yes" in g_wall_opt and not disable_struct:
        g_wall_qty = st.number_input(f_lang("Glass Wall Qty / 玻璃幕墙数量"), 1, 10, 1)
        bill.append({"Cat": t_cat("结构"), "Item": t_item("玻璃幕墙"), "Spec": "Yes", "Qty": g_wall_qty, "RMB": get_p('玻璃幕墙', size)})
    
    if "Yes" in st.selectbox(f_lang("Outdoor Terrace+Roof / 室外露台+屋顶"), ["No / 不需要", "Yes / 需要"], disabled=disable_struct, format_func=f_lang):
        bill.append({"Cat": t_cat("结构"), "Item": t_item("露台"), "Spec": "Yes", "Qty": 1, "RMB": get_p('室外露台+屋顶', size)})
        
    if "Yes" in st.selectbox(f_lang("Stairs / 楼梯"), ["No / 不需要", "Yes / 需要"], disabled=disable_struct, format_func=f_lang):
        bill.append({"Cat": t_cat("结构"), "Item": t_item("楼梯"), "Spec": "Yes", "Qty": 1, "RMB": get_p('楼梯', size)})
        
    is_frozen_roof = disable_struct or disable_special
    if "Yes" in st.selectbox(f_lang("Full Roof / 通铺屋顶"), ["No / 不需要", "Yes / 需要"], disabled=is_frozen_roof, format_func=f_lang):
        bill.append({"Cat": t_cat("结构"), "Item": t_item("通铺屋顶"), "Spec": "Yes", "Qty": 1, "RMB": get_p('通铺屋顶', size)})

with c3:
    h_rod_qty = st.number_input(f_lang("Hydraulic Rod Qty / 液压杆数量 (0-4)"), 0, 4, 0, disabled=disable_struct)
    ...
    if "Yes" in st.selectbox(f_lang("Trailer / 拖车"), ["No / 不需要", "Yes / 需要"], disabled=is_frozen_trailer, format_func=f_lang):
        bill.append({"Cat": t_cat("结构"), "Item": t_item("拖车"), "Spec": "Yes", "Qty": 1, "RMB": get_p('拖车', size)})
    
    qty_foot = st.number_input(f_lang("Foot Cups / 地脚杯 (Qty)"), 0, 20, 0, disabled=is_x)
    if qty_foot > 0: bill.append({"Cat": t_cat("配件"), "Item": t_item("地脚杯"), "Spec": "Yes", "Qty": qty_foot, "RMB": get_p('螺栓可调节支撑地脚杯', size)})
    
    qty_leg = st.number_input(f_lang("Support Legs / 支撑腿 (Qty)"), 0, 20, 0, disabled=is_x)
    if qty_leg > 0: bill.append({"Cat": t_cat("配件"), "Item": t_item("支撑腿"), "Spec": "Yes", "Qty": qty_leg, "RMB": get_p('可调节大支撑腿', size)})

# --- 6. Top & Skirting ---
st.markdown("---")
st.subheader(f_lang("6. Top & Skirting / 顶部与踢脚"))
...
with c1:
    ...
    top_opts = st.multiselect(f_lang("Top Config / 顶部配置"), raw_top_opts, disabled=disable_top_all, format_func=f_lang)
    for t in top_opts:
        t_cn = get_cn(t)
        bill.append({"Cat": t_cat("装修"), "Item": f_lang(t), "Spec": "Yes", "Qty": 1, "RMB": get_p(t_cn, size)})

with c2:
    skirt = st.selectbox(f_lang("Skirting / 踢脚线"), ['No / 无', 'PVC', 'Mn-Al / 锰铝合金'], disabled=disable_top_all, format_func=f_lang)
    skirt_cn = get_cn(skirt)
    if skirt_cn != '无' and not disable_top_all: 
        bill.append({"Cat": t_cat("装修"), "Item": t_item("踢脚线"), "Spec": f_lang(skirt), "Qty": 1, "RMB": get_p(skirt_cn, size)})

# --- 7. Certification ---
st.markdown("---")
st.subheader(f_lang("7. Certification / 认证"))
c1, c2, c3 = st.columns(3)
with c1:
    opt_wire = st.selectbox(f_lang("Wire Cert / 电线认证"), ["No / 不需要", "Yes / 需要"], format_func=f_lang)
    if "Yes" in opt_wire:
        std = st.selectbox(f_lang("Wire Std / 标准"), ['EU / 欧标', 'US / 美标', 'AU / 澳标'], format_func=f_lang)
        bill.append({"Cat": t_cat("认证"), "Item": t_item("认证电线"), "Spec": f_lang(std), "Qty": 1, "RMB": get_p('认证电线', size)})
with c2:
    if "Yes" in st.selectbox(f_lang("Socket Cert / 插座认证"), ["No / 不需要", "Yes / 需要"], format_func=f_lang):
        bill.append({"Cat": t_cat("认证"), "Item": t_item("插座认证"), "Spec": "Yes", "Qty": 1, "RMB": get_p('认证插座开关', size)})
    if "Yes" in st.selectbox(f_lang("Plumbing Cert / 上下水认证"), ["No / 不需要", "Yes / 需要"], disabled=is_x, format_func=f_lang):
        bill.append({"Cat": t_cat("认证"), "Item": t_item("上下水认证"), "Spec": "Yes", "Qty": 1, "RMB": get_p('上下水认证', size)})
with c3:
    if "Yes" in st.selectbox(f_lang("Light Cert / 灯具认证"), ["No / 不需要", "Yes / 需要"], format_func=f_lang):
        bill.append({"Cat": t_cat("认证"), "Item": t_item("灯具认证"), "Spec": "Yes", "Qty": 1, "RMB": get_p('认证灯', size)})

    opt_toilet_cert = st.selectbox(f_lang("Toilet / 马桶"), ["No / 不需要", "Yes / 需要"], disabled=is_x, format_func=f_lang)
    if "Yes" in opt_toilet_cert and not is_x:
        bill.append({"Cat": t_cat("认证"), "Item": f_lang("Toilet / 马桶"), "Spec": "Yes", "Qty": 1, "RMB": get_p('马桶', size)})
...
if not df_res.empty:
    ...
    st.header(f"💰 {f_lang('Total Price / 预估总价')}: $ {total_usd:,.2f}")
    
    st.markdown(f"#### ({f_lang('EXW Price / 出厂价')} | {f_lang('Price Validity / 价格有效期至')}: {valid_date})")
    
    df_display = df_res[['Cat', 'Item', 'Spec', 'Qty']].rename(columns={
        "Cat": f_lang("Category / 类别"), 
        "Item": f_lang("Item / 项目"), 
        "Spec": f_lang("Spec / 规格"), 
        "Qty": f_lang("Qty / 数量")
    })
    ...
    if IS_ADMIN:
        ...
        with st.expander(f_lang("📄 Configuration List / 配置清单"), expanded=True):
            st.table(df_display)

    st.caption(f"🚢 {f_lang('FOB Price / FOB 价格')} : $ {fob_price:,.2f}")

else:
    st.info(f_lang("Please select items to generate quote. / 请选择配置以生成报价。"))
